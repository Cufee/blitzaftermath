    ...
    def calculate_rating(self):
        ...
        for player in self.replay_data.get('players'):
            ...
            damage_efficiency = ((damage_made + damage_assisted) / tank_hp_avg) * \
                self.eff_multiplyers.get(tank_type).get('damage_efficiency')

            kill_efficiency = (kills / 7) * \
                self.eff_multiplyers.get(tank_type).get('kill_efficiency')

            travel_efficiency = (distance_travelled / travel_avg) * \
                self.eff_multiplyers.get(tank_type).get('travel_efficiency')

            shot_efficiency = ((shots_penetrated) /
                               (shots_fired)) * \
                self.eff_multiplyers.get(tank_type).get('shot_efficiency')

            spotting_efficiency = ((enemies_spotted) / (players_count / 2)) * \
                self.eff_multiplyers.get(tank_type).get('spotting_efficiency')

            blocked_efficiency = (damage_blocked / tank_hp_avg) * \
                self.eff_multiplyers.get(tank_type).get('blocked_efficiency')

            track_efficiency = (damage_assisted_track / tank_hp_avg) * \
                self.eff_multiplyers.get(tank_type).get('track_efficiency')

            rating = round(((damage_efficiency + kill_efficiency +
                             travel_efficiency + shot_efficiency + spotting_efficiency + track_efficiency + blocked_efficiency) / 7 * 1000))
